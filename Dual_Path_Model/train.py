...
from network import model_single_path,model_dual_path
...
class Train(object):
    def __init__(self,config,train_loader,valid_loader,test_loader):
        ...
        self.fine_tuning = config.fine_tuning_or_not

        # Path
        self.model_path = config.model_path
        self.model_type = config.model_type
        self.sampling_num=config.sampling_number
    ...
    def build_model(self,):
        ...
        if self.model_type == 'single_path':
            self.model = model_single_path()
        elif self.model_type == 'dual_path':
            self.model = model_dual_path(out_ch=128)

        if self.fine_tuning==True:
            # This opreation needs to freeze the parameters (except the mlp module) of Dual-Path model first
            self.model.load_state_dict(torch.load('.\checkpoint\model-dualpath_update1_temporal_prior_equ_var-maxsampling-6000-1000-4.4%(last_epoch).pth'))
            print('---------------------------------paremeters is successfully loaded----------------------------------')
            self.optimizer = optim.AdamW(filter(lambda p: p.requires_grad, self.model.parameters()),self.lr)
        else:
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if reerror_test/len(self.test_loader) < min_error_test:
                ...
                path = self.model_path + '-' + self.model_type + '-' + '-' + str(
                    self.impact_window) + '-' + str(self.sampling_num) + \
        ...
        # path=self.model_path+'-'+self.model_type+'-'+self.sampling_type+'-'+str(self.impact_window)+'-'+str(self.sampling_num)+\
        #                     '-'+'%.1f'%(round(float(min_error_test), 3) * 100)+'%.pth'
        path1=self.model_path+'-'+self.model_type+'-'+'-'+str(self.impact_window)+'-'+str(self.sampling_num)+\
